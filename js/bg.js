var c = document.createElement('canvas');
var ctx = c.getContext("2d");
document.body.appendChild(c);
window.onresize = resize;
//document.body.onmouseover = render;
document.body.onmouseout = resize;
//document.body.onmousemove = render;
$(document).mousemove(function  (a) {
    var x = a.pageX;
    var y = a.pageY;
    var w = $(document).width();
    var h = $(document).height();
    if(x > 200 || x < (w - 200) || x > 200 || x < (w - 200)){
        document.body.onmouseover = render;
        document.body.onmousemove = render;
    }else{
        document.body.onmouseover = resize;
        document.body.onmousemove = resize;
        resize();
    }
})
document.body.onload = resize;

/*-----------------------*/

var dots = [];
var colW = 50;
var lineH = 50;
var nbCols;
var nbLines;
var nbDots;
var startT = new Date().getTime();
var t = 0;
var mouse = {x: window.innerWidth * 0.5, y: window.innerHeight * 0.5, startX: 0, startY: 0, destX: 0, destY: 0, time: 0, totalTime: 0};
var action = {x: window.innerWidth * 0.5, y: window.innerHeight * 0.5};
var zoneRadius;
var zoneStep = 100;

var Dot = function(x, y)
{
    this.x = x;
    this.y = y;
    this.ax = x;
    this.ay = y;
};

function createDots()
{
    var w = c.width;
    var h = c.height;
    nbCols = ~~(w / colW) + 2;
    nbLines = ~~(h / lineH) + 2;
    nbDots = nbCols * nbLines;
    var decalX = -17;
    var decalY = -10;
    dots = [];

    for(var i = 0; i < nbLines; i++)
    {
        for(var j = 0; j < nbCols; j++)
        {
            dots.push(new Dot(decalX + j * colW, decalY + i * lineH));
        }
    }
}

function drawDots(anchors, color, radius)
{
    var dot;
    for(var i = 0; i < nbDots; i++)
    {
        dot = dots[i];
        dist = getDistance(dot, action);
        ctx.globalAlpha = Math.max(1 - (dist / (zoneRadius * 1.2)), 0);;
        ctx.beginPath();
        if(anchors)
        {
            ctx.moveTo(dot.ax, dot.ay);
            ctx.arc( dot.ax, dot.ay, radius, 0, Math.PI * 2, true);
        }
        else
        {
            ctx.moveTo(dot.x, dot.y);
            ctx.arc( dot.x, dot.y, radius, 0, Math.PI * 2, true);
        }
        ctx.closePath();
        ctx.fillStyle = color;
        ctx.fill();
    }
}

function drawLines(color)
{
    var dot, nextDot, col, lin, dist;
    for(var i = 0; i < nbDots; i++)
    {
        line = ~~(i / nbCols);
        col = i % nbCols;

        dot = dots[i];
        dist = getDistance(dot, action);
        ctx.globalAlpha = Math.max(1 - (dist / (zoneRadius * 1.5)), 0.05);
        ctx.beginPath();
        if(line < (nbLines - 1))
        {
            nextDot = dots[i + nbCols];
            ctx.moveTo(dot.x, dot.y);
            ctx.lineTo(nextDot.x, nextDot.y);
        }
        if(col < (nbCols - 1))
        {
            nextDot = dots[i + 1];
            ctx.moveTo(dot.x, dot.y);
            ctx.lineTo(nextDot.x, nextDot.y);
        }
        ctx.closePath();
        ctx.strokeStyle = color;
        ctx.stroke();
    }
}

function drawJoints(color)
{
    var dot, nextDot, col, lin, dist;
    for(var i = 0; i < nbDots; i++)
    {
        dot = dots[i];
        dist = getDistance(dot, action);
        ctx.globalAlpha = Math.max(1 - (dist / (zoneRadius * 1.5)), 0.05);
        ctx.beginPath();

        ctx.moveTo(dot.x, dot.y);
        ctx.lineTo(dot.ax, dot.ay);

        ctx.closePath();
        ctx.strokeStyle = color;
        ctx.stroke();
    }
}

function getDistance(dot1, dot2)
{
    return Math.sqrt((dot2.x - dot1.x) * (dot2.x - dot1.x) + (dot2.y - dot1.y) * (dot2.y - dot1.y));
}

function moveDots()
{
    var dot, dist, angle;
    for(var i = 0; i < nbDots; i++)
    {
        dot = dots[i];
        angle = -Math.atan2(action.x - dot.ax, action.y - dot.ay) - (Math.PI * 0.5);
        dist = getDistance(dot, action);

        if(dist <= zoneRadius)
        {
            dot.x = dot.ax + zoneStep * (1 - (dist / zoneRadius)) * Math.sin(angle);
            dot.y = dot.ay + zoneStep * (1 - (dist / zoneRadius)) * Math.cos(angle);
        }
        else
        {
            dot.x = dot.ax;
            dot.y = dot.ay;
        }
    }
}

function resize()
{
    var box = c.getBoundingClientRect();
    var w = box.width;
    var h = box.height;
    c.width = w;
    c.height = h;
    zoneRadius = Math.min(c.width * 1.2, 250);
    createDots();
    renderout();
}

function render(e)
{
    mouse.x = e.pageX;
    mouse.y = e.pageY;

    ctx.clearRect(0, 0, c.width, c.height);
    action.x += (mouse.x - action.x) * 0.07;
    action.y += (mouse.y - action.y) * 0.07;
    moveDots();
    drawLines("transparent");
    drawJoints("#fff");
    drawDots(true, "#F00", 2);//dots
}

function renderout()
{
    mouse.x = 0;
    mouse.y = 0;
    ctx.clearRect(0, 0, c.width, c.height);
    action.x += (mouse.x - action.x) * 0.07;
    action.y += (mouse.y - action.y) * 0.07;
    drawLines("transparent");
}
